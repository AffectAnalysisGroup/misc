...
from bigram_trigram_mapping import bigram_mapping, trigram_mapping, map_affect_codes
...
def calculate_bigram_histogram(families, depressed_families, criterion='construct_code', data=data, depression_labels=True):
    '''
    '''
    ...
    for family in families: # For each family
        ...
        for idx in range(len(fam_annotations)-1): #data['construct_code'][min(fam_annotations):max(fam_annotations)-1]):
            curr_con = data[criterion][fam_annotations[0]+idx]
            next_con = data[criterion][fam_annotations[0]+idx+1]
...
def calculate_trigram_histogram(families, depressed_families, data=data, criterion='construct_code', depression_labels=True):
    '''
    Calculate the trigram histogram.
    The histogram index is inferred from the trigram_mapping_ generated form bigram_trigram_mapping.py file
    '''
    print('using {0} families'.format(len(families)))
    labels = []
    ...
    for family in families: # For each family
        ...
        for idx in range(len(fam_annotations)-2): #data['construct_code'][min(fam_annotations):max(fam_annotations)-1]):
            t0_con = data[criterion][fam_annotations[0]+idx]
            t1_con = data[criterion][fam_annotations[0]+idx+1]
            t2_con = data[criterion][fam_annotations[0]+idx+2]
...
if __name__ == '__main__':
    ...
    for fam in families:
        ...
        # print(fam)
    # print(count)
    # modified_data = map_affect_codes(data)
    bi_hist, bigram_data, bigram_labels = calculate_bigram_histogram(families, depressed_families)#, data=modified_data)
    tri_hist, trigram_data, trigram_labels = calculate_trigram_histogram(families, depressed_families)#, data=modified_data)
    # print(tri_hist)
    ...
    model.classify_and_predict(optimize_for='kappa', grid_verbose=False, njobs=-1, folds=5)
